...
/etc/nginx/sites-available/<_project_name_>_backend.conf
```ts 
# <_project_name_> Backend API Configuration
server {
    ...
    # Domain or subdomain this server block will respond to
    ...
    location / {
        ...
        # Forward requests to backend application running on <_port_>
        proxy_pass http://localhost:<_port_>;


        # Enable HTTP/1.1 for WebSockets and long connections
        proxy_http_version 1.1;


        # Required headers to support WebSockets
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";


        # Pass original domain and client IP to backend
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;


        # Prevent caching for API traffic
        proxy_cache_bypass $http_upgrade;
    }


    # Store logs for easier debugging
    access_log /var/log/nginx/<_project_name_>_backend_access.log;
    error_log  /var/log/nginx/<_project_name_>_backend_error.log;
}

```

## 🟩 2. ADMIN DASHBOARD CONFIG
> File: /etc/nginx/sites-available/<_project_name_>_admin.conf

```ts 
# <_project_name_> Admin Dashboard Configuration
server {
    ...
    # Domain for admin dashboard (example: admin.example.com)
    server_name <_sub_domain_>.<_domain_>;


    # Path where admin build files are stored
    root /var/www/<_project_name_>/admin/build;


    # Default file for SPA applications
    index index.html;


    # Handle SPA routing (React, Vue, Svelte)
    location / {
        # If file exists → serve it, else serve index.html
        try_files $uri /index.html;
    }


    # Route API requests from admin to backend server
    location /api/ {
        proxy_pass http://localhost:<_port_>;
    ...
    # Admin-specific access/error logs
    access_log /var/log/nginx/<_project_name_>_admin_access.log;
    error_log  /var/log/nginx/<_project_name_>_admin_error.log;
}

```

## 🟧 3. FRONTEND WEBSITE CONFIG

> File: /etc/nginx/sites-available/<_project_name_>_website.conf
